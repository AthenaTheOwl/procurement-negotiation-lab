"""Validate dream records (dreams/<YYYY-WNN>/) in procurement-negotiation-lab.
...
# Env-var override mirrors the validate_decisions.py / validate_skills.py
# pattern so the offline-cache code path stays testable end-to-end.
REMOTE_URL = os.environ.get("PROCUREMENT_LAB_SCHEMA_URL_BASE", DEFAULT_REMOTE_URL)
...
def load_remote_schema() -> dict[str, Any] | None:
    try:
        req = urllib.request.Request(  # noqa: S310
            REMOTE_URL,
            headers={"User-Agent": "procurement-lab/validate_dreams"},
        )
