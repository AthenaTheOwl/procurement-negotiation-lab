"""Validate role records (.agents/roles/<role-id>/role.yaml) against the
cross-repo `role.schema.json` from athena-site.
...
`ops/schemas-cache/role.schema.json` so CI runs offline.
...
def load_remote_schema() -> dict[str, Any] | None:
    try:
        req = urllib.request.Request(  # noqa: S310
            REMOTE_URL,
            headers={"User-Agent": "procurement-lab/validate_roles"},
        )
...
def discover_roles() -> list[Path]:
    ...
    return sorted(
        path
        for path in ROLES_DIR.glob("*/role.yaml")
        if path.is_file()
    )
...
def main() -> int:
    ...
    try:
        ...
        raise SystemExit(
            "validate_roles: PyYAML is required. "
