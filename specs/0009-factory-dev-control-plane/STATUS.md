...
| R-FACTORY-RUN-EVIDENCE-018 | COVERED | [DEC-FACTORY-010](../../decisions/DEC-FACTORY-010-procurement-lab-portable-repo-uri-migration.md) | `scripts/finalize_sandbox_ref.py` rewrites the PENDING placeholder to the sample-containing SHA; idempotent. Sample `run-960d6b107160` carries the finalized URI. | None for the in-scope behavior. |
...
| R-FACTORY-RUN-EVIDENCE-026 | COVERED | [DEC-FACTORY-013](../../decisions/DEC-FACTORY-013-factory-thread-id-capture-and-timestamp-fix.md) | `WorkerResult.metadata` carries `thread_id`, `run_id`, `model`, `duration_ms`, `tokens_input`, `tokens_output` on every worker invocation (real or stub). Missing IDs synthesize as `<label>-cli-<uuid12>` and `<label>-run-<uuid12>`. Typed accessors return `None` for missing keys. | Checkpoint, interrupt, and `--resume` defer to a future commit per the deferred addendum-6 scope. |
| R-FACTORY-RUN-EVIDENCE-027 | COVERED | [DEC-FACTORY-013](../../decisions/DEC-FACTORY-013-factory-thread-id-capture-and-timestamp-fix.md) | `ClaudeCodeWorker` and `CodexWorker` try `--output-format json` first and fall back to plain `--print`/`exec` on detected unsupported-flag stderr. Token-count extractor parses Anthropic + OpenAI usage blocks under top-level and nested paths. `StubWorker(seed=...)` produces deterministic IDs. | None for the in-scope behavior. |
| R-FACTORY-RUN-EVIDENCE-028 | COVERED | [DEC-FACTORY-013](../../decisions/DEC-FACTORY-013-factory-thread-id-capture-and-timestamp-fix.md) | `scripts/replay_run.py::_now_iso_filename` uses microsecond resolution so back-to-back replays inside the same wall-clock second never collide on the ledger filename. Existing glob patterns accept both formats. | None for the in-scope behavior. |
| R-FACTORY-RUN-EVIDENCE-029 | COVERED | [DEC-FACTORY-014](../../decisions/DEC-FACTORY-014-procurement-negotiation-lab-chaos-test-suite.md) | `tests/factory/test_chaos_run_evidence.py` carries a positive guard plus seven mutation tests (M1..M7) that copy the canonical sample into `tmp_path`, apply one targeted mutation, redirect the validator's path constants, and assert exit code != 0 with stderr naming the right check. | None for the in-scope behavior. |
| R-FACTORY-RUN-EVIDENCE-030 | COVERED | [DEC-FACTORY-014](../../decisions/DEC-FACTORY-014-procurement-negotiation-lab-chaos-test-suite.md) | `test_M5_pipeline_start_missing_prompt_hash_is_caught` mutates the `pipeline.start` payload on a temp-dir ledger copy; the event schema's oneOf discriminator rejects the line at schema-validation time. | None for the in-scope behavior. |
| R-FACTORY-RUN-EVIDENCE-031 | COVERED | [DEC-FACTORY-014](../../decisions/DEC-FACTORY-014-procurement-negotiation-lab-chaos-test-suite.md) | `.github/workflows/run-evidence-gates.yml` declares a `chaos-validation` job that runs the suite under `uv`; the job's proof tokens are registered in `scripts/spec_check.py::REQUIRED_WORKFLOW_PROOFS` so a renamed or deleted job fails spec-check. | None for the in-scope behavior. |
...
- [DEC-FACTORY-013](../../decisions/DEC-FACTORY-013-factory-thread-id-capture-and-timestamp-fix.md): addendum-6 emission slice extends `WorkerResult.metadata` with `thread_id` + `run_id` + `model` + `duration_ms` + `tokens_input` + `tokens_output` on every worker invocation; `ClaudeCodeWorker` and `CodexWorker` try `--output-format json` first; `scripts/replay_run.py::_now_iso_filename` switches to microsecond resolution so the replay-determinism fixture's back-to-back replays never collide on the ledger filename.
- [DEC-FACTORY-014](../../decisions/DEC-FACTORY-014-procurement-negotiation-lab-chaos-test-suite.md): chaos test suite at `tests/factory/test_chaos_run_evidence.py` copies the canonical sample into `tmp_path`, applies one targeted mutation per test from a seven-class catalog (M1..M7), and asserts `scripts/validate_run_evidence.py` exits non-zero with stderr naming the right check; the `chaos-validation` job in `.github/workflows/run-evidence-gates.yml` runs the suite in CI.
