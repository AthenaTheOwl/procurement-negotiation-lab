# traceability: weighted-Nash transcript-exposure bargaining
...
| **R-NASH-004** transcript-exposure iteration protocol (owner_role: science.proof-gate-runner) | A2, A3, A4 | privacy.py invoked when information_mode=private; messages match protocol spec; deterministic given seed | planned |
| **R-NASH-005** transcript-exposure measurement (owner_role: science.proof-gate-runner) | A2, A3, A6, D4 | Per-run exposure report with exposure bits + round count + message log hash; property test R-PROP-006 green | planned |
| **R-NASH-006** transcript-exposure report schema (owner_role: engineering.implementation) | A3, A6 | TranscriptExposureReport Pydantic model + JSON Schema mirror; run-evidence emitter validates | planned |
...
| **R-NASH-008** cryptographic MPC second mechanism (owner_role: science.proof-gate-runner) | D1, D2, D3, D4, E1, E2, E3 | MPC mechanism returns same allocation as plaintext within tolerance; exposure report records negligible-function parameter | planned |
...
| **R-NASH-010** per-run record references mechanism + exposure (owner_role: engineering.implementation) | A3, A6 | run record carries mechanism_id + leakage_report_ref; mechanism.bargaining.completed event present; replay-determinism gate covers all mechanisms | planned |
