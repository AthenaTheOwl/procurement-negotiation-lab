...
- [ ] **A1**: Add `alpha?: number` to `LabScenario` in `web/src/model/types.ts`.
  Default 1.0. *(R-OPS-001)*
- [ ] **A2**: Add `vcgTransfer(scenario, agentId, alpha)` to `simulation.ts`
  and route the cpp-vcg mechanism through it. *(R-OPS-001)*
- [ ] **A3**: Update `transferLedger(scenario)` signature to accept an
  optional `{ alpha }` and pass through to vcgTransfer. *(R-OPS-001)*
- [ ] **A4**: Lab Arena: add α slider on the mechanism config panel.
  Live-update the transfer ledger on change. *(R-OPS-001)*
- [ ] **A5**: Arc Step 3: add a small α slider plus a one-paragraph
  explanation that α weakens dominant-strategy incentive compatibility
  while preserving anti-gaming pressure. *(R-OPS-001)*
- [ ] **A6**: Unit tests in `simulation.test.ts` covering α=0, α=0.5,
