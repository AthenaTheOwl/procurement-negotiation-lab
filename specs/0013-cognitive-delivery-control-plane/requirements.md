...
Spec 0013 installs the Cognitive Delivery Control Plane (CDCP) governance
scaffold in procurement-negotiation-lab. The 12 prior specs already name
the what. This spec adds the parallel records that name the why
(decisions), the what-we-reuse (skills, roles, tools, policies, state
machines, workflows), and the what-we-learned (dreams), plus the
executable gates that fail builds when any of those records drift out
of shape.

The CDCP shape itself lives in the cross-repo charter at
`../athena-site/ops/control-plane.md`. This repo references those
...
- An allowlist file at `decisions/.spec-check-allowlist.yaml` lists the
  legacy R-* IDs whose backfill DECs land in a later pass; the gate skips
...
- The script keeps a local cache copy at
  `ops/schemas-cache/decision.schema.json` so CI runs offline without a
  network fetch.
...
Acceptance:
- A future `scripts/validate_dreams.py` (lands with the first real dream
  output in a later pass) walks `dreams/**/output.json` and validates.
- `dreams/README.md` documents the eight dream modes and the human-gate
  rule for promotion candidates.
...
- The current backfill records the 20 most recent pre-CDCP commits
  through `646d989`.
- Each entry names which gates the release passed.
- `ops/RELEASE_LEDGER.md` is distinct from the factory `ops/run-ledger.md`;
  the factory ledger tracks per-task pipeline runs, the release ledger
  tracks commit-level releases.
...
- The initial state records "No resets recorded." until the first reset.
- Operators do not silently rewrite history; the ledger entry lands in
  the same push that performs the rewrite.
...
- The agent contract points at the gate scripts under `scripts/` and at
  the factory orchestrator under `scripts/factory/`.
...
Acceptance:
- `.agents/skills/run-factory-task/SKILL.md` graduates the existing
  `scripts/factory/` orchestrator-worker pattern.
- The SKILL.md front-matter matches the cross-repo `skill.schema.json`
  shape.
- Promotion past the initial version requires `human_approval` per the
  promotion_policy field.

### R-CDCP-008: CI gate failures block merge to main

WHEN any of `spec_check`, `voice_lint`, `validate_decisions`,
`validate_roles`, `validate_tools`, or `validate_policies` fails, THE
SYSTEM SHALL fail the CI job and block the PR from merging.

Acceptance:
- `.github/workflows/tests.yml` runs all six python gates in the test
  job.
- The frontend, mobile-e2e, smoke, and security workflows keep their
  existing scopes; the CDCP gates ride alongside the python tests.
- A failed gate causes the GitHub check to fail, blocking merge under
...
- No CI job auto-applies a dream candidate; the rule is structural.
...
- `scripts/validate_decisions.py`, `scripts/validate_roles.py`,
  `scripts/validate_tools.py`, and `scripts/validate_policies.py`
  fetch their schemas by URL with a local cache fallback.
