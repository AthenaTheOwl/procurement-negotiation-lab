...
decision: |
  procurement-negotiation-lab ships the transcript-exposure weighted-
  Nash protocol at ``src/procurement_lab/engine/privacy.py``. The
  ...
  value. Transcript exposure is measured per run and reported through a
  stable TranscriptExposureReport schema. Historical names such as
  ``leakage_report``, ``epsilon_measured``, and this DEC's file name
  remain as compatibility labels; they are not differential-privacy
  claims.

  Terminology amendment, 2026-06-18: earlier text in this DEC used
  "bounded-leakage" and "preference-private" as shorthand. Current
  code and public copy should use "transcript exposure" for the
  iterative protocol. The measured quantity is an upper bound on
  transcript information content in bits. It is not a calibrated
  privacy budget and should not be described as DP.
  ...
  - ``protocol_version: str`` — pinned to ``"transcript-exposure/v1"`` in
    this DEC
  ...
  ## Transcript-exposure model

  Per-run transcript exposure for party ``p`` is bounded by the information
  ...
  - ``step_proposal`` carries up to ``log_2(STEP_QUANTIZATION_LEVELS)
    = 5`` bits per round
  - Over ``R`` rounds: ``exposure_bits_measured_p <= R * (N * log_2(3) + 5)``
  ...
  ``N = 4`` gives ``exposure_bits_measured_p <= 50 * (4 * 1.585 + 5) = 567``
  ...
  ``epsilon_measured <= epsilon_bound`` per the legacy schema field names below.

  Comparison points:
  - The plaintext solver (DEC-NASH-001) reveals every party's full
    utility to the centralized aggregator; full-transcript exposure per
    party. This is the exposure baseline the transcript-exposure protocol
    improves on.
  - The MPC mechanism (DEC-MPC-001, W5) achieves negligible transcript
    exposure under cryptographic security parameter ``lambda``. It is
    the cryptographic comparison point by construction.

  The protocol does not claim differential privacy. The exposure
  ...
  ## TranscriptExposureReport schema
  ...
  Fields:
  - ``protocol_version: str`` — must equal ``"transcript-exposure/v1"``
  ...
  - ``per_party: list[TranscriptExposureParty]`` where the serialized legacy alias ``PartyLeakage`` has:
    - ``party_id: str``
  ...
  - ``aggregate: AggregateTranscriptExposure`` where the serialized legacy alias ``AggregateLeakage`` has:
  ...
  sequence and the same TranscriptExposureReport. The replay-determinism gate
  in ``.github/workflows/run-evidence-gates.yml`` covers runs that
  use the transcript-exposure mechanism.

  ## Run-evidence integration

  The run record (per DEC-NASH-001 R-NASH-010 coverage) carries:
  - ``mechanism_id = "weighted_nash_bounded"`` for transcript-exposure runs
  ...
  per run with payload
  ``{mechanism_id, exposure_aggregate.max_epsilon_measured}``; the
  nested field name remains compatible with existing consumers.
alternatives:
  - label: send per-allocation utility values in the messages
    rejected_because: |
      ...
      via interpolation. The protocol's exposure claim — that the
  ...
  - label: continuous-real-valued direction vectors instead of ternary
    rejected_because: |
      ...
      precision; the exposure bound becomes meaningless. Ternary
  ...
  - label: claim differential-privacy guarantees
    rejected_because: |
      ...
      privacy budget tracked across composed mechanisms. The transcript-exposure protocol does neither: it bounds the transcript's
      information content, which is a weaker guarantee. Claiming DP
      would overstate the protocol's properties. The
      ``sufficiency_note`` field in the TranscriptExposureReport makes the
      contract honest.
  - label: omit the TranscriptExposureReport from the per-run record
    rejected_because: |
      Without TranscriptExposureReport in the run record, replay cannot verify
      that the protocol's exposure contract held on a given run. The
  ...
  - label: ship without ``protocol_version`` field
    rejected_because: |
      ...
      distinguish exposure reports from the two versions. The
...
rationale: |
  ...
  transcript-exposure measurement. Spec 0015 R-NASH-006 names the report schema.
  All three need a concrete decision before code lands; without one,
  the implementation could ship an exposure claim that does not
  ...
  The transcript-exposure bound is information-theoretic, not differential
  ...
  but explicit accounting contract — an adversary observing the full transcript
  ...
  The MPC mechanism (DEC-MPC-001, W5) is the cryptographic
  alternative for users who need cryptographic guarantees. DEC-NASH-002
  names it as a comparison point; the transcript-exposure protocol is the
  ...
  visible; clients that consume the TranscriptExposureReport schema break if
  the schema changes. A follow-up DEC revising the protocol bumps
  ``protocol_version`` from ``"transcript-exposure/v1"`` to a new value
...
rollback: |
  ...
  ``TranscriptExposureReport`` and ``TranscriptExposureParty`` Pydantic models from
...
systems_map: |
  Transcript-exposure accounting vs cryptographic guarantee - a
  protocol claiming limited disclosure without naming what is
  transmitted, what is not, and how much information the transcript
  can reveal risks overclaiming a guarantee the implementation does
  not provide.
  DEC-NASH-002 closes that gap by fixing the message shape, the
  exposure formula, and the report schema. The protocol's disclosure
  property is the bound, not "no information leaks"; the
  ``sufficiency_note`` field makes the bound's looseness visible to
  the consumer of the TranscriptExposureReport.
transferable_principle: |
  Any disclosure-limiting protocol must (a) name what is and is not
  ...
  the protocol's exposure claim is unfalsifiable. This pattern applies
...
falsification_test: |
  ...
  test against the transcript), the transcript-exposure accounting claim is
...
adoption_ladder:
  minimum_viable: |
    ``privacy.py`` implements the protocol; ``TranscriptExposureReport`` schema
  ...
  mid_adoption: |
    ...
    ``leakage_report_ref`` on every transcript-exposure run; replay-
    determinism gate covers transcript-exposure runs; the
    ``mechanism.bargaining.completed`` event type lands.
  full_adoption: |
    NegotiateSurface (spec 0016) UI exposes the TranscriptExposureReport to the
  ...
  monitoring_signals:
    - "transcript-exposure property test pass/fail trend on main"
    - "per-run max_epsilon_measured distribution on real runs"
    - "delta between transcript-exposure final allocation and plaintext on golden fixtures"
    - "TranscriptExposureReport schema-version drift"
    - "MAX_ROUNDS hit rate (protocol failing to converge in bound)"
---

## decision

procurement-negotiation-lab implements the transcript-exposure weighted-Nash protocol at ``src/procurement_lab/engine/privacy.py``. The
...
transcript exposure is measured against an information-theoretic upper bound and reported through a stable TranscriptExposureReport schema mirrored from a
...
- ``R-NASH-005`` transcript-exposure measurement: per-run formula
  ``epsilon_measured = R * (N * log_2(3) + 5)`` per party.
- ``R-NASH-006`` TranscriptExposureReport schema: Pydantic model in
