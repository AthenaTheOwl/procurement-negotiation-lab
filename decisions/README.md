...
decision resolves. Example: `DEC-MOBREL-001-eas-build-profiles.md`
resolves `R-MOBREL-001`. The slug is the short title in kebab-case.

## Adding a new decision

1. Identify the R-* requirement the decision resolves.
2. Copy `DEC-CDCP-001-install-cdcp-governance.md` as a starting
   template.
...
The repo ships exactly one DEC in this pass:

- `DEC-CDCP-001-install-cdcp-governance.md` — the meta-decision to
  install the CDCP scaffold.

The 91 pre-CDCP R-* requirements (across specs 0001-0012) are listed
in `decisions/.spec-check-allowlist.yaml` as deferred backfill. Each
cluster (R-PLAY-*, R-LAB-*, R-STUDY-*, R-SPEC-*, R-ARC-*, R-OPS-*,
R-PORTAL-*, R-REPORT-*, R-HARDEN-*, R-BRIDGE-*, R-FACTORY-*,
R-LEARN-*, R-MOBILE-*, R-MONO-*, R-SANDBOX-*, R-GUARD-*, R-MOBREL-*,
R-SDLC-*, R-AIBRIEF-*) lands in a later pass, one cluster at a time.
